import PropertySlider from "react-slick";
import './style.css'
...
export const MarkerInfoWindow = ({
    ...
    return (
        <div className="info-window">
            <a href={`/property/${mlsForLink}`}>
                <PropertySlider {...settings}>
                    {imageSlider && imageSlider.map((image, index) => (
                        <div key={index}>
                            <img src={`https://cdn.repliers.io/${image}?class=small`}
                                 alt={`Image ${index + 1}`}/>
                        </div>
                    ))}
                </PropertySlider>
                ...
                <div className={'p-2 mb-1'}>
                    <div className={'d-flex justify-content-between align-items-center'}>
                        <h5 className={'pb-0'}>{price}</h5>  <small className={'badge bg-light text-dark'}>{pType} for
                        Sale</small>
                    </div>
                    <p className={'pb-0'}>
                        {address.unitNumber !== "" ? `${address.unitNumber} - ` : ''}{address.streetNumber}&nbsp;
                        {(address.streetDirectionPrefix !== "" && address.streetDirectionPrefix !== null && address.streetDirectionPrefix !== undefined) ? `${address.streetDirectionPrefix} ` : ''}
                        {address.streetName}&nbsp;
                        {(address.streetSuffix !== "" && address.streetSuffix !== null && address.streetSuffix !== undefined && address.streetSuffix !== '0') ? `${address.streetSuffix}, ` : ''}
                        {address.city},&nbsp;
                        {address.state}.
                    </p>
                    <ul className={'hg-amin-card d-flex mt-2 mb-0'}>

                        {(bed !== "" && bed !== null && bed !== undefined && bed !== '0') ?
                            <li className={'me-3'}>
                                <svg height={'1.3em'} className={'me-1'}
                                ...
                                {bed}{(bedPlus !== "" && bedPlus !== null && bedPlus !== undefined && bedPlus !== '0') ? ` + ${bedPlus} ` : ''} bed(s)
                            </li> : ''}


                        {(bath !== "" && bath !== null && bath !== undefined && bath !== '0') ?
                            <li className={'me-3'}>
                                <svg height={'1.25em'} className={'me-1'} viewBox="0 0 24 24" fill="none"
                        ...
                        {(numOfGarage !== "" && numOfGarage !== null && numOfGarage !== undefined && parseFloat(numOfGarage) !== 0) ?
                            <li>
                                <svg height={'1.2em'} className={'me-1'} xmlns="http://www.w3.org/2000/svg"
                                     viewBox="0 0 640 512">
                                    <path
                                ...
                                {parseFloat(numOfGarage)} Garage(s)
                            </li> : ''}
                    </ul>
                </div>
            </a>
